(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('nearDocsApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-nearDocsApp-error');
                var entityKey = httpResponse.headers('X-nearDocsApp-params');
                if (errorHeader) {
